...
Миграции создают DocTypes (**Pulse User Profile**, **Pulse Session Event**), поле **`presence_source`**, Custom Field на **User** (**Pulse last seen**), а также **Workspace «Pulse»** и плитку приложения на экране Desk (через **`add_to_apps_screen`** и синхронизацию **Desktop Icon**, как в типичной настройке Frappe v15/v16).

После установки на экране приложений (**Apps**) должна появиться плитка **Pulse** с переходом на **`/app/pulse`** (ярлыки на профили присутствия, журнал сессий, пользователей). Если плитки нет — выполните **`migrate`** ещё раз и **`bench restart`**, затем **Очистить кэш** в Desk или `bench --site … clear-cache`.
...
2. В поиске типов документов найдите **Pulse User Profile** и **Pulse Session Event** — записи появятся после первых действий присутствия.
3. Откройте **User** — должно быть поле **Pulse last seen** (если Custom Field создался после migrate).
...
3. Обновляются профиль Pulse и поле **Pulse last seen** у User.
...
| Где | Что |
|-----|-----|
| **User** | Поле **Pulse last seen**, блок **Connections** к связанным записям Pulse (если включены связи). |
| **Pulse User Profile** | Детали присутствия по пользователям, поле **Presence source**. |
...
| Поле **Presence source** / колонка не видна | Повторно **`bench migrate`**, **Customize Form** для User — наличие Custom Field **Pulse last seen**. |
