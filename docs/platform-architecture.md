...
- Просмотр онлайна и сводки по меткам источника (**`client_service`**) через HTTP того же сервиса: см. **`presence_ws/README.md`** (раздел **«Примеры curl»** — `GET /services`, `GET /online`, фильтр `?client_service=`).
