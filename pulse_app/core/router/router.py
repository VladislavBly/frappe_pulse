...
def _normalize_dispatch_path(path: str) -> str:
	p = (path or "").strip()
	if not p.startswith("/"):
		p = "/" + p
	return p.rstrip("/") or "/"
...
class Router:
	"""HTTP route registry: path + methods → handler (Werkzeug Map)."""
		...
		*,
		...
		def decorator(fn):
			...
			if ep in self._handlers:
				raise ValueError(f"Router: duplicate endpoint name {ep!r}")
			self._rules.append(
	...
	def dispatch(self, path: str, method: str | None = None) -> object:
		...
		path = _normalize_dispatch_path(path)
